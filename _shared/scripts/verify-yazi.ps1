...
if ($yaziCmd) {
    ...
    try { $v = (yazi --version) 2>&1 | Select-Object -First 1; Write-Output "version: $v" }
    catch { $reasons += "yazi exists but failed to run"; Write-Output "version: FAILED to run" }
} else {
...
if ($yaziCmd) {
    try {
        $dbg = (yazi --debug 2>&1 | Select-Object -First 30 | Out-String)
        if ($dbg -match "TOML parse error" -or $dbg -match "Press <Enter> to continue") {
    ...
    } catch { Write-Output "config: could not run yazi --debug" }
}

# 3. YAZI_FILE_ONE (previews)
$yfo = [Environment]::GetEnvironmentVariable("YAZI_FILE_ONE", "User")
...
$endpoints = @("https://github.com", "https://raw.githubusercontent.com")
$netOk = $true
